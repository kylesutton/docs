...
<Accordion title="RUN_COMMAND_ARGS">
...
<table class="table w-full [&_td]:min-w-[150px] [&_th]:text-left [&_td[data-numeric]]:tabular-nums">
  ...
  <tr>
    ...
    <td>If only one input table is needed, it can be passed by query or by name to the <code>INPUT_TABLE</code> function; e.g.:: INPUT_TABLE(customer) INPUT_TABLE(SELECT id, name FROM customer)</td>
  </tr>
  <tr>
    <td><code>INPUT_TABLES(&lt;queries&gt;)</code></td>
    <td>If more than one input table is needed, each can be passed via query as a list to the <code>INPUT_TABLES</code> function; e.g.:: INPUT_TABLES ( (SELECT id, name FROM customer), (SELECT id, customer_id FROM order) )</td>
  </tr>
  </tbody>
</table>
</div>
...
<Accordion title="&lt;environment name&gt;">
