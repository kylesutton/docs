...
| --- | --- | --- |
| `-f </path/to/file>` <br/> `--path </path/to/file>` | N/A | User-specified directory in which to create the control files. Default directory is `/opt/gpudb/api/python/` |
| `-p <param_name value>` <br/> `--param <param_name value>` | N/A | UDF execution parameter(s) (see [/execute/proc](/content/api/rest/execute_proc_rest#execute-proc-rest) for more information) |
| `-d` <br/> `--distributed` | N/A | Enable distributed UDF simulation. This is the default execution mode |
| `-i <table_name [column_name, ...]>` <br/> `--input <table_name [column_name, ...]>` | Distributed only | Input table and optional column list |
| `-o <table_name>` <br/> `--output <table_name>` | Distributed only | Output table |
| `-n` <br/> `--nondistributed` | N/A | Enable non-distributed UDF simulation |
| `-K <url>` <br/> `--url <url>` | N/A | Kinetica URL; default is `http://localhost:9191` |
| `-U <username>` <br/> `--username <username>` | N/A | Kinetica username for authentication |
| `-P <password>` <br/> `--password <password>` | N/A | Kinetica password for authentication |
| `-h` <br/> `--help` | N/A | Prints the help menu |
...
|                                      |                                                                                    |
| `--dry-run`                        |                                                                                    |
| `-K <url>` <br/> `--url <url>` | Kinetica URL; default is `http://localhost:9191` |
| --- | --- |
| `-U <username>` <br/> `--username <username>` | Kinetica username for authentication |
| `-P <password>` <br/> `--password <password>` | Kinetica password for authentication |
| `-h` <br/> `--help` | Prints the help menu |
