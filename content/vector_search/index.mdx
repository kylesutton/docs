...
| `MODEL_NAME` | Name of the *model* to use in generating embeddings, adhering to the supported [naming criteria](/content/sql/naming#sql-naming-criteria); if not specified, the default model, `sqlgpt`, will be used. <br/> To set the default to another model and avoid having to specify this parameter: <br/> {/* Source: /examples/sql/vector/embeddings.kisql (lines 168) */} <br/> `ALTER SYSTEM SET PROPERTIES ('ai_api_embeddings_model' = 'nvidia_remote_model')` |
