...
| `MASK(column, start, length[, char])` | Masks `length` characters of `column`, beginning at the position identified by `start`, with `*` characters (or the character specified in `char`): <br/> <br/> `MASK('Characters', 5, 5)`: `Char*****s` <br/> `MASK('Characters', 5, 2, '#')`: `Char##ters` |
