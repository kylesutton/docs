...
| **sql.planner.address** | `ipc://${gaia.temp_directory}/gpudb-query-engine-0` | The network URI for the query planner to start. The URI can be either TCP or IPC. TCP address is used to indicate the remote query planner which may run at other hosts. The IPC address is for a local query planner. <br/> <br/> Example for remote or TCP servers: <br/> <br/> `sql.planner.address  = tcp://127.0.0.1:9293 sql.planner.address  = tcp://HOST_IP:9293` <br/> <br/> Example for local IPC servers: <br/> <br/> `sql.planner.address  = ipc:///tmp/gpudb-query-engine-0` <br/> |
