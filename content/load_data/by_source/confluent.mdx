---
title: Load Data from Confluent Kafka
description: Copy and paste tutorial for loading data from Confluent Kafka
sidebarTitle: Kafka (Confluent)
---

<a id="load-data-by-source-confluent"></a>
...
<a id="load-from-confluent-credential"></a>
...
<a id="load-from-confluent-data-source"></a>
...
<a id="load-from-confluent-ingest"></a>
...
{/* Source: /examples/sql/load/load_into_format_confluent.sql (lines 3-5) */}
```sql Start Ingest
...
{/* Source: /examples/sql/load/load_into_format_confluent.sql (lines 10-11) */}
```sql Pause Ingest
ALTER TABLE example.orders
PAUSE SUBSCRIPTION confluent_ds
```

{/* Source: /examples/sql/load/load_into_format_confluent.sql (lines 16-17) */}
```sql Resume Ingest
ALTER TABLE example.orders
RESUME SUBSCRIPTION confluent_ds
```

{/* Source: /examples/sql/load/load_into_format_confluent.sql (lines 22-23) */}
```sql Cancel Ingest
ALTER TABLE example.orders
...
{/* Source: /examples/sql/load/load_into_confluent_snippets.sql (lines 59-65) */}
```sql Abort
...
{/* Source: /examples/sql/load/load_into_confluent_snippets.sql (lines 69-76) */}
```sql Log Bad Records
LOAD INTO ki_home.error_example
FORMAT JSON
WITH OPTIONS (
...
{/* Source: /examples/sql/load/load_into_confluent_snippets.sql (lines 7-21) */}
...
{/* Source: /examples/sql/load/load_into_confluent_snippets.sql (lines 25-36) */}
```sql Custom Date
-- Example Data:
-- {
--   "d": "(2022) Feb, 22"
...
{/* Source: /examples/sql/load/load_into_confluent_snippets.sql (lines 40-51) */}
```sql Custom Time
-- Example Data:
-- {
--   "t": "18-27-59.5536"
...
{/* Source: /examples/sql/load/load_into_confluent_snippets.sql (lines 84-95) */}
```sql Null String
-- Example Data:
-- {
--   "example_null": "null"
...
<a id="load-from-confluent-schema-registry"></a>
...
{/* Source: /examples/sql/credential/credentials_format_confluent.kisql (lines 82-91) */}
```sql Create Confluent Credentials
CREATE CREDENTIAL confluent_cred
TYPE = 'confluent'
WITH OPTIONS
(
...
{/* Source: /examples/sql/credential/credentials_format_confluent.kisql (lines 118-126) */}
...
{/* Source: /examples/sql/load/load_into_format_confluent.sql (lines 39-46) */}
```sql Initiate Load
LOAD DATA INTO example.orders
FORMAT AVRO
WITH OPTIONS
