...
<p><strong>Command-Line Startup</strong></p>
...
* If `external_text_search = true` in the `/opt/gpudb/core/etc/gpudb.conf`
  file, then the text search server can be managed using the directives
  above. If `external_text_search = false`, then the text search server
  will not be able to be managed individually.
